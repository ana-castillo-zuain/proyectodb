...
with st.sidebar:
    ...
    st.write("Quick links")
    default_page = st.session_state.get("page", "Home")
    pages = ["Home", "Series", "Watch Parties", "Trending", "Platforms", "My Watchlist", "Party Lobby"]
...
if page == "Party Lobby":
    ...
    if not wp_id:
        st.warning("No party selected. Enter the lobby in the Watch Parties section.")
    else:
        ...
        if not wp:
            st.error("❌ Could not find this Watch Party in the database.")
        else:
            ...
            st.markdown(f"**Hosted by:** {wp.get('host', '—')}")
            st.markdown(f"**Time:** {wp.get('time', '—')}")
            st.markdown(f"**Platform:** {wp.get('platforms', '—')}")
            st.markdown(f"**Participants:** {', '.join(participants) or '—'}")

            if st.button("⬅ Back to Watch Parties"):
...
if page == "Trending":
    ...
    st.markdown("<div class='trend-title'>⭐ Top Rated</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='trend-title'>👥 Friends’ Picks</div>", unsafe_allow_html=True)
...
if page == "Platforms":
    ...
    for name in plats:
        try:
            ...
            st.error(f"Error loading platform {name}: {e}")
